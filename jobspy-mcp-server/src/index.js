...
const ENABLE_SSE = ['1', 'true', 'yes'].includes(
  String(process.env.ENABLE_SSE || '').toLowerCase(),
);
    ...
    if (ENABLE_SSE) {
      try {
        ...
        app.post('/api', async (req, res) => {
          try {
            const rawParams = req.body?.params ?? req.body;
            const params = changeCase.camelCase(rawParams);
